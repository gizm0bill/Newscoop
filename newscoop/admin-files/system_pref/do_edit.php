...
$f_max_author_image_filesize = strip_tags(Input::Get('f_max_author_image_filesize'));
...
// Max Author Image File Size
$max_author_image_filesize_bytes = camp_convert_bytes($f_max_author_image_filesize);
if ($max_author_image_filesize_bytes > 0 &&
        $max_author_image_filesize_bytes <= min(camp_convert_bytes(ini_get('post_max_size')), camp_convert_bytes(ini_get('upload_max_filesize')))) {
    SystemPref::Set("MaxAuthorImageFileSize", $f_max_author_image_filesize);
} else {
    $msg_ok = 0;
    camp_html_add_msg(getGS('Invalid Max Upload File Size value submitted'));
}
