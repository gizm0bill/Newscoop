...
<div class="article-padding edit-profile-tab">
    <article>
        ...
        <fieldset class="fixBackground">
            <h3 class="checkHeading">E-Mail-Adresse bestätigt.</h3>
            <p>Bitte vervollständigen Sie nun Ihre Zugangsdaten, um Ihr Benutzerkonto zu aktivieren. Mit dem Benutzerkonto können Sie Ihr Abo online verwalten, eigene Beiträge veröffentlichen und mit anderen TagesWoche-Lesern in Kontakt treten.</p>
            <ul>
                <li><dl>
                    {{ $form->first_name->setLabel("Vorname*")->removeDecorator('Errors') }}
                    {{ if $form->first_name->hasErrors() }}
                    <span class="error-info">Bitte geben Sie eine Vorname an.</span>
                    {{ /if }}
                </dl></li>
                <li><dl>
                    {{ $form->last_name->setLabel("Nachname*")->removeDecorator('Errors') }}
                    {{ if $form->last_name->hasErrors() }}
                    <span class="error-info">Bitte geben Sie eine Nachname an.</span>
                    {{ /if }}
                </dl></li>
                <li><dl>
                    {{ $form->username->setLabel("Nutzername*")->removeDecorator('Errors') }}
                    {{ if $form->username->hasErrors() && !$form->username->getValue() }}
                    <span class="error-info">Bitte geben Sie eine Nutzername an.</span>
                    {{ else if $form->username->hasErrors() }}
                    <span class="error-info">Nutzername besetzt. Bitte geben Sie eine andere Nutzername an.</span>
                    {{ /if }}
                ...
                <li><dl>
                    ...
                    <span class="error-info">Bitte geben Sie eine Passwort mindestens 6 Zeichen lang an.</span>
                    {{ /if }}
                    <span class="input-info">Ihre Passwort solite mindestens 6 Zeichen lang sein</span>
                </dl></li>
                <li><dl>
                    ...
                    <span class="error-info">Bitte geben Sie seine Passwort noch einmal an.</span>
                    {{ /if }}
                    <span class="input-info">Bitten geben Sie Ihr passwort erneut ein, um Tippfehler auszuschliesen</span>
                </dl></li>
                {{ if ($terms == true) }}
                <li>
...
34. Auf alle Rechtsbeziehungen aus dem Vertrag zwischen der Betreiberin und dem Nutzer sowie diesen AGB ist Schweizerisches Recht unter Ausschluss der Bestimmungen des UN-Kaufrechts anwendbar. Ausschliesslicher Gerichtsstand ist Basel als der Sitz der Betreiberin. Die Betreiberin kann allerdings Klagen gegen den Nutzer auch an dessen Sitz oder Wohnsitz anhängig machen.</textarea>
                ...
                <li><dl>
                    {{ $form->terms_of_use->setLabel("Ich habe die Nutzungsbedingungen gelesen und stimme Ihnen zu.*")->removeDecorator('Errors') }}
        ...
        <span class="form-buttons left">
                Felfer mit * sind Pflichtfelder
            </span>
        <div class="form-buttons right">
                <!--<button class="button">Konto aktivieren</button>//-->
                <input type="submit" class="button" value="Konto aktivieren" />
            </div>
        </fieldset>
